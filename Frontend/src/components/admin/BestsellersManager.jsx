...
export default function BestsellersManager({ onBack }) {
  // ---- State ----
  const [featured, setFeatured] = useState([]);      // current bestsellers (sorted)
  const [allProducts, setAllProducts] = useState([]); // for picker
  ...
  const loadFeatured = useCallback(async () => {
    ...
    setError('');
    try {
      // Admin version — return ALL bestsellers (even out-of-stock/inactive)
      const res = await api.get('/products/admin/products', {
        params: { badgeType: undefined, limit: 100 },
      });
      ...
      try {
        const res = await api.get('/products/bestsellers', { params: { limit: 50 } });
        setFeatured(res.data?.data || []);
      } catch (err) {
        setError(err?.response?.data?.message || 'Failed to load bestsellers');
      }
    } finally {
      setLoading(false);
    }
  ...
  // ---- Toggle bestseller via API ----
  const toggleBestseller = async (productId) => {
    try {
      const res = await api.patch(
        `/products/admin/products/${productId}/toggle-bestseller`
      );
  ...
  const handleAdd = async (product) => {
    ...
    setSaving(true);
    const updated = await toggleBestseller(product._id);
    setSaving(false);

    if (updated) {
      // Append optimistically (server assigns order)
  ...
  const handleRemove = async (product) => {
    if (!confirm(`Remove "${product.name}" from bestsellers?`)) return;
  ...
  const handleDragEnd = async (result) => {
    ...
    const reordered = arr.map((p, i) => ({ ...p, bestsellerOrder: i }));
    setFeatured(reordered);

    try {
      await api.patch('/products/admin/products/reorder-bestsellers', {
        order: reordered.map(({ _id, bestsellerOrder }) => ({
          id: _id,
          order: bestsellerOrder,
        })),
      });
    } catch (e) {
    ...
    <div className="bg-white rounded-xl shadow-sm border border-pink-100 p-6">
      ...
      <div className="flex items-center justify-between mb-6">
        <div className="flex items-center gap-3">
          ...
          <div>
            <h2 className="text-2xl font-bold text-slate-800">Bestsellers</h2>
            <p className="text-sm text-slate-500">
              Pick which products appear in the homepage Bestsellers section
            </p>
          </div>
        </div>
        <button
      ...
      {loading ? (
        ...
        <div className="text-center py-20 text-slate-500 border-2 border-dashed rounded-xl">
          No bestsellers yet. Click <strong>Add Product</strong> to feature some.
        </div>
      ) : (
        <DragDropContext onDragEnd={handleDragEnd}>
          <Droppable droppableId="bestsellers">
            {(provided) => (
              <div
                      ...
                      >
                        ...
                        {isFeatured ? (
                          <span className="text-xs text-pink-600 font-semibold">
                            <Star className="w-4 h-4 fill-yellow-400 text-yellow-500 inline" />
                          </span>
                        ) : (
